    ...
    while(True):
        try:
            ...
            if(seq==0 and data[0:7]=='\x01\x01'+HANDSHAKE):
                ...
                obj={'Num':sockNum,'InQueue':Queue.Queue(),'OutQueue':Queue.Queue(),'Address':addr[0],'Identifier':id,'CanSend':False,'HeartBeatFlag':_GetMd5(str(sockNum)+'OPEN'),'CloseTCPFlag':_GetMd5(str(sockNum)+'CLOSE') }
                obj['Identifier']=id
